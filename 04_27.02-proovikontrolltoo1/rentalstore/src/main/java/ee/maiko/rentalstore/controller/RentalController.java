...
import ee.maiko.rentalstore.dto.FilmRentalDto;
...
import ee.maiko.rentalstore.repository.RentalRepository;
...
public class RentalController {
    ...
    //DTO -- mis film, mitmeks p2evaks v6tan
    @PostMapping("start-rental")
    public Rental startRental(@RequestBody List<FilmRentalDto> filmRentalDtos){
        Rental rental = new Rental(); // id null
        Rental dbRental = rentalRepository.save(rental); // id 2
        ...
        dbRental.setInitialFee(sum); // id 2 initalFee 123 lateFee null
        return rentalRepository.save(dbRental);
    }

    //DTO -- mis film, mitu p2eva tegelikult rendis oli
    ...
    public double endRental(@RequestBody List<FilmRentalDto> filmRentalDtos){
        ...
        for (FilmRentalDto filmRentalDto: filmRentalDtos){
            ...
            rental.setLateFee(rental.getLateFee()+FILMI_SUMMA); // <-- v]ibolla switchi sees
